...
/**
 * Processor which processes the payment instruments.
 * 
 * @author Karthik.
 * 
 */
@Service("PYMNTAUTHCREATE")
public class PaymentInstrumentPreloadProcessor extends AbstractBatchFileProcessor {

    private static Logger logger = LoggerFactory.getLogger(PaymentInstrumentPreloadProcessor.class);
    ...
    @Autowired
    @Qualifier("paymentInstrumentPreload")
    private Job job;
    ...
    protected BatchFileProcessingResult doProcess(File file) {
        ...
        try {
            ...
            String fileType = BatchOutputFileName.PYMNTAUTHCREATECONF.name();
            ...
            logger.warn("Error while executing payment instrument preload for file: {} ", file.getAbsolutePath(), e);
            retVal.setSuccess(false);
        }
        return retVal;
    }
